...
const plotRadarGraph = function (title, blips, currentRadarName, alternativeRadars) {
  document.title = 'Kloia Technology Radar'
  ...
  // Show description text and hide loading after radar is plotted
  if (featureToggles.UIRefresh2022) {
    const descriptionElement = document.querySelector('.helper-description > p')
    const loaderElement = document.querySelector('.helper-description .loader-text')
  ...
  self.build = function () {
    ...
    window.addEventListener('keydown', function (e) {
      if (featureToggles.UIRefresh2022 && e.key === '/') {
        ...
        if (document.activeElement !== inputElement && inputElement) {
          e.preventDefault()
          inputElement.focus()
    ...
    // Always load Kloia's radar data automatically
    const kloiaRadarUrl = 'https://plum-barbie-93.tiiny.site/radar.csv'
    sheet = CSVDocument(kloiaRadarUrl)
    sheet.init().build()
  }
...
function plotLoading(content) {
  if (!featureToggles.UIRefresh2022) {
    ...
    var bannerText =
      '<h1>Visualizing the radar...</h1><p>Your Technology Radar will be available in just a few seconds</p>'
